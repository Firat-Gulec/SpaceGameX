...
class CandyScene: SKScene, SKPhysicsContactDelegate {
    ...
    var candyCategory:UInt32 = 0x1 << 1
    ...
    extension CandyScene {
        ...
        func createPlayer() {
            ...
            player.physicsBody?.contactTestBitMask = candyCategory
        ...
        func createCandy() {
            var candy: Candy
            let type: CandySettings
            let duration : CGFloat
            
            switch Int(arc4random() % 100) {
            case 0...25:
                candy = Candy.createCandySmall()
                type = .small
                duration = CGFloat(Float(arc4random() % 3) + 2.0)
            case 25...50:
                candy = Candy.createCandySmall2()
                type = .small
                duration = CGFloat(Float(arc4random() % 3) + 2.0)
            case 50...75:
                candy = Candy.createCandyMedium2()
                type = .small
                duration = CGFloat(Float(arc4random() % 3) + 3.0)
            case 75...80:
                candy = Candy.createCandyMedium()
                type = .medium
                duration = CGFloat(Float(arc4random() % 3) + 3.0)
            case 80...90:
                candy = Candy.createCandyLarge2()
                type = .medium
                duration = CGFloat(Float(arc4random() % 3) + 5.0)
            default:
                candy = Candy.createCandyLarge()
                type = .large
                duration = CGFloat(Float(arc4random() % 3) + 5.0)
            }
            candy.type = type
            let candyF = candy.frame
            let randomX = CGFloat.random(min: candyF.width, max: self.frame.width - candyF.width )
            candy.position = CGPoint(x:randomX, y: self.frame.height + candyF.height / 2 )
            ...
            candy.physicsBody?.contactTestBitMask = bulletCategory
            ...
            candy.run(.repeatForever(.sequence([ group,  .removeFromParent()])))
        }
        ...
        func createBullet() {
            ...
            bullet.physicsBody?.contactTestBitMask = candyCategory
        ...
        func addLives() {
            ...
            for live in 1...3 {
                let liveNode = SKSpriteNode(imageNamed: "Spaceship_candy")
    ...
    //MARK: - SKPhysicsContactDelegate
        ...
        func didBegin(_ contact: SKPhysicsContact) {
            ...
            if (firstBody.categoryBitMask & bulletCategory) != 0 && (secondBody.categoryBitMask & candyCategory) != 0 {
            
                let candy = contact.bodyA.categoryBitMask == candyCategory ? contact.bodyA.node as? Candy: contact.bodyB.node as? Candy
                ...
                if let candy = candy {
                    ...
                    if candy.healty < 0 {
                        candy.healty = 0
                        print("bitti")
                        changeScore(candy.type)
                        bulletDidCollideWithCandy(candyNode: secondBody.node as! SKSpriteNode )
                    }
                    if candy.healty == 0 {
                        changeScore(candy.type)
                        bulletDidCollideWithCandy(candyNode: secondBody.node as! SKSpriteNode )
            ...
            if (firstBody.categoryBitMask & playerCategory) != 2 && (secondBody.categoryBitMask & candyCategory) != 2 {
                //Crash SpaceShip
                playerDidCollideWithCandy(playerNode: firstBody.node as! SKSpriteNode)
        ...
        //player fire explosion
         func playerDidCollideWithCandy (playerNode:SKSpriteNode) {
       ...
       //bullet fire explosion
        func bulletDidCollideWithCandy (candyNode:SKSpriteNode) {
            let explosion = SKEmitterNode(fileNamed: "bulling")!
            explosion.position = candyNode.position
            addChild(explosion)
            run(SKAction.playSoundFileNamed("explosion.mp3", waitForCompletion: false))
        ...
        func changeScore (_ type:CandySettings) {
            switch type {
            case .small:
                score += 100
            case .medium:
